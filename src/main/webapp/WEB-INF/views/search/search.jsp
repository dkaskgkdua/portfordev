...
<html>
   <head>
      ...
      .img{vertical-align:super;margin-top: 3px;}
      .context_text {vertical-align:top;}
      </style>
   ...
   <body class="subpage">
      ...
      <!-- Three -->
         ...
         <c:if test="${searchKeyword!=''}">
            ...
            <h1 class="profile_h1 search_h1 sub" id="project">PORTFOLIO</h1>
               ...
               <c:forEach var = "p" items="${portResult}">
                  <li>
                     <div class="contentimg divimg"><img class="img" src="resources/${p.PORT_THUMBNAIL}"/></div>
                     <div class="context_text">
                      <div id="div_sub" class="PORT_SEARCH">
                         <input type="hidden" value="${p.PORT_ID}" class="hidden_PORT_ID">
                        <a href="#" class="PORT_SUBJECT">${p.PORT_SUBJECT}</a>
                      </div>   
               ...
               <c:forEach var="memberList" items="${memberResult}">
                  ...
                  <!-- 일등급 -->
                  <c:if test="${memberList.MEMBER_ACT>400}">
                     <img src="resources/Image/icon/level_1.png" style="width:30px;"/>
                  </c:if>   
                  <!-- 이등급 -->
                  <c:if test="${memberList.MEMBER_ACT >300 && memberList.MEMBER_ACT <400}">
                     <img src="resources/Image/icon/level_2.png" style="width:30px;"/>
                  </c:if>   
                  <!-- 삼등급 -->
                  <c:if test="${memberList.MEMBER_ACT >200 && memberList.MEMBER_ACT <300}">
                     <img src="resources/Image/icon/level_3.png" style="width:30px;"/>
                  </c:if>   
                  <!-- 사등급 -->
                  <c:if test="${memberList.MEMBER_ACT >100 && memberList.MEMBER_ACT <200}">
                     <img src="resources/Image/icon/level_4.png" style="width:30px;"/>
                  </c:if>   
                  <!-- 오등급 -->
                  <c:if test="${memberList.MEMBER_ACT >=0 && memberList.MEMBER_ACT <100}">
               ...
               <c:if test="${!empty boardResult}">
                <c:forEach var="boardList" items="${boardResult}">
                   <li>
                     <a href="board_view_action?board_id=${boardList.BOARD_ID}">${boardList.BOARD_SUBJECT}</a><span id="date">${boardList.BOARD_DATE}</span>
                     <br>
                     <div class="context_text">
               ...
               <li class="more"><a href="board_list?search_select=3&BOARD_CATEGORY=0&sort=DATE&search_text=${searchKeyword}">More ></a></li>
               </c:if>   
               ...
               <c:forEach var = "studyList" items="${studyResult}">
                  <li>
                     <a href="board_view_action?board_id=${boardList.BOARD_ID}">${studyList.BOARD_SUBJECT}</a><span id="date">${studyList.BOARD_DATE}</span>
                     <br>
                     <div class="context_text">
                  ...
                  <li class="more"><a href="board_list?search_select=3&BOARD_CATEGORY=1&sort=DATE&search_text=${searchKeyword}">More ></a></li>
               </c:if>
               <c:if test="${empty studyResult}">
               ...
               <c:if test="${!empty qnaResult}">
                  <c:forEach var="qnaList" items="${qnaResult}">
                     <li>
                     <a href="board_view_action?board_id=${boardList.BOARD_ID}">${qnaList.BOARD_SUBJECT}</a><span id="date">${qnaList.BOARD_DATE}</span>
                     <br>
                     <div class="context_text">
                  ...
                  <li class="more"><a href="board_list?search_select=3&BOARD_CATEGORY=2&sort=DATE&search_text=${searchKeyword}" >More ></a></li>
               </c:if>
               <c:if test="${empty qnaResult}">
