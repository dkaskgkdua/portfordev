$(window).load(function(){
	...
	$(window).resize(function(){
		var targetHeight = $('.portfolio-detail-project').height() - 115;
		if(targetHeight >= 283)
			$('.slideitem-img').css('height', targetHeight + 'px');
		else
			$('.slideitem-img').css('height', '283px');
		...
		success: function(port){
			...
			if(port.PORT_IMG_FILES == '')
			{
				...
				$.each(portImgs, function(){
					...
					slideItem += 	'</div>';
					$('.port-slide-list').append(slideItem);
					$('.slideitem-img').css('height', ($('.portfolio-detail-project').height() - 115) + 'px');
				});	
				doSlick();
			}
			...
			if(port.PORT_GITHUB == null || port.PORT_GITHUB == '')
				$('.port-github-img').css('display', 'none');
			else{
				$('.port-github-href').val(port.PORT_GITHUB);
				$('.port-github-img').css('display', 'inline-block');
			}
			// 포트폴리오 사이트 주소
			if(port.PORT_SITE == null || port.PORT_SITE == '')
				$('.port-site-img').css('display', 'none');
			else{
				$('.port-site-href').val(port.PORT_SITE);
				$('.port-site-img').css('display', 'ilnine-block');
			}
			if(port.PORT_GITHUB == null && port.PORT_SITE == null)
...
function showDetail(){
	...
	/*$('body').addClass('not-scroll').on('scroll touchmove mousewheel', function(){
        return false
    }, {passive: false});*/
...
function doSlick()
{
	...
	$('.port-slide-item').on('click', function(){
		...
		$('#portfolio-pdf-enlargement-wrap').animate({scrollTop : 0}, 100, function(){
